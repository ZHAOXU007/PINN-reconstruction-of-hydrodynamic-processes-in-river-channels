...
select_location = {x_min,1400, 2800, 4200, 5600, 7000, 8400, 9800, 11200, 12600,x_max}
t_step2 = 200
# Load model
model = PINN()
model.net.load_state_dict(torch.load('model_k1000.param'))
...
# Define functions
def h_acu(x, t):
    return 54.5 - 40 * x / 14000 - 10 * torch.sin(torch.pi * (4 * x / 14000 - 0.5)) - 4 * torch.sin(torch.pi * (4 * t / 86400 + 0.5))

def u_acu(x, t):
    return torch.pi * (x - 14000) / 5400 / h_acu(x, t) * torch.cos(torch.pi * (4 * t / 86400 + 0.5))

def z_acu(x):
    return 10 + 40 * x / 14000 + 10 * torch.sin(torch.pi * (4 * x / 14000 - 0.5))
