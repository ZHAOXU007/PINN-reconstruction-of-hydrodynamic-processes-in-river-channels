...
class PINN:
    ...
    def h_acu(self, x,t):
        h = 54.5-40*x/14000-10*torch.sin(torch.pi*(4*x/14000-0.5))-4*torch.sin(torch.pi*(4*t/86400+0.5))
        return h

    def u_acu(self, x,t):
        u = torch.pi*(x-14000)/5400/self.h_acu(x,t)*torch.cos(torch.pi*(4*t/86400+0.5))
        return self.k*u
    ...
    def get_training_data(self):
        ...
        inlet_x = x_min * np.ones_like(inlet_t)
        ...
        outlet_x = x_max * np.ones_like(outlet_t)
        ...
        garph_t = 1500*np.ones_like(garph_x)
        ...
        self.garph_u = self.u_acu(self.garph_xt[:, 0:1], self.garph_xt[:, 1:2]).to(device)
    ...
    def loss_obs(self):
        ...
        return  torch.mean(torch.square(h_pred1 - self.bound1_h)) + torch.mean(torch.square(u_pred1 - self.bound1_u))\
               ...
               +torch.mean(torch.square(h_pred8 - self.bound8_h)) + torch.mean(torch.square(u_pred8 - self.bound8_u))\
               +torch.mean(torch.square(h_pred9 - self.bound9_h)) + torch.mean(torch.square(u_pred9 - self.bound9_u))
    ...
    def loss_B2(self):
        ...
        return torch.mean(torch.square(h_pred - self.outlet_h)) + torch.mean(torch.square(u_pred - self.outlet_u))+ torch.mean(torch.square(z_pred - self.outlet_z))
    ...
    def closure(self):
        ...
        loss = 500*loss_pde + loss_obs  + loss_boundary
    ...
    def plot_current_state(self):
        ...
        with torch.no_grad():
            ...
            u = self.net(self.garph_xt)[:, 1:2].detach().cpu().numpy()  # 使用 .detach()
...
if __name__ == "__main__":
    ...
    torch.save(pinn.net.state_dict(), f'model_k{pinn.k}.param')
    # 将记录的数据保存到 Excel
    df = pd.DataFrame(pinn.recorded_data)
    df.to_excel(f'training_loga_k{pinn.k}.xlsx', index=False)